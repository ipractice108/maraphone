...
def job_thu1(chat_id):
    bot.send_message(chat_id, '🌝Добрейшего утра!) \n'
                              ...
                              '\n'
                              '🌬Сегодня твой день "ДЕНЬ ОСОЗНАННОГО ДЫХАНИЯ". Вытянись пальцами руки и пальцами ног в разные стороны вытягивая позвоночник.  Определи ногу с которой ты встаешь и вперед на утренние процедуры.')
# почистить зубы
...
def job_thu3(chat_id):
    bot.send_message(chat_id, '🥛➕🥛➕🍋➕👌➕👌Пьем стакан воды комнатной температуры натощак и за ним второй с щепоткой курскумы и чистым лимонным соком и содой.')

def job_thu33(chat_id):    
      bot.send_message(chat_id, '🥛➕🥛Если захочется есть, пьем еще воду, стараемся позавтракать первый раз в 13.00')
...
def job_thu6(chat_id):
      bot.send_message(chat_id, '🌾➕🥗Гречка с овощами.\n'
                                ...
                                '\n'
                                '🕵️‍♀️Находим подходящий вариант гречки, варим 15-20 мин на средней интенсивности жара. Или заливаем простой водой на более длительное время, так в нашей повседневной обжаренной гречке, сохранится больше полезных свойств.\n'
                                '\n'
...
def job_thu66(chat_id):
    bot.send_message(chat_id, 'https://youtu.be/QguteGdSkkI')

# evening lesson
def job_thu7(chat_id):
    bot.send_message(chat_id, '🗓И конечно же помним, крайний прием пищи до 18.00. Обнял, приподнял)')

def job_thu77(chat_id):
