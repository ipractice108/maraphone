...
def job_tue3(chat_id):
    bot.send_message(chat_id,  '🥛Выпейте стакан воды комнатной температуры натощак и за ним сразуже второй.\n'
                                              'ВИДЕО ГДЕ ПЬЮ ДВА СТАКАНА ВОДЫ')

def job_tue33(chat_id):
    bot.send_message(chat_id,  '🥛➕🥛➕🥛Если захочется есть, попейте еще воды, постарайтесь позавтракать первый раз в 12.30')
...
def job_tue7(chat_id):
    bot.send_message(chat_id,  '🗓Напоминаю, крайний прием пишщи до 18.00')
...
def job_tue9(chat_id):
    bot.send_message(chat_id,  '🙋🏽‍♂Доброй ночи, надеюсь Вам понравилось! Завтра ждет третий день марафона и новая программа!')
# -------------------------------------------------------------------------------------------------------------------------
#TUESDAY SCHEDULE
