...
def job_tue4(chat_id):
    bot.send_message(chat_id,  'https://youtu.be/EAxW2ivMkpw')
    bot.send_message(chat_id, '🕙В 10.00 ты получишь нопоминание о режиме питания + рецепт завтрака')

        # warm up + food notice

def job_tue5(chat_id):
    bot.send_message(chat_id,  '🍧➕🍌Каша с фруктами.\n'
                                        ...
                                        '\n'
                                        '👨‍🍳Рецепт - отмеряем порцию каши размером с кулак и заливаем кипятком, разбавляя на глаз до жидкообразного состояния. Сладкие фрукты, сухофрукты, и масло по вкусу !в очень умеренных количествах!')

    bot.send_message(chat_id, '🕛В 12.00 ты получишь рецепт обеда')
        # lounch recipie

def job_tue6(chat_id):
    bot.send_message(chat_id,  '🍘➕🥗Булгур с овощами.\n'
                                            '\n'
                                            ...
                                            '\n' 
                                            '🙏Пожалуйста постарайся воздержаться от еды начиная с 19.00. 		И конечно же если вечером перед сном ты выпьешь немного мятного или травяного чаю с минимальным количеством натуральных сладостей, ничего критичного не произойдет. \n'
                                            '\n'
                                            '➡Вечером я пришлю следующее видео с практикой.')

def job_tue66(chat_id):
    bot.send_message(chat_id, 'https://youtu.be/RRJvEdQ5_7M')

# evening lesson

def job_tue7(chat_id):
    bot.send_message(chat_id,  '🗓Напоминаю, крайний прием пищи до 19.00')

def job_tue77(chat_id):
    bot.send_message(chat_id,  '😋Заканчиваем есть!) Мини перекусы тоже считаются!)')



def job_tue8(chat_id):
    bot.send_message(chat_id,  'https://youtu.be/FsLWLBmdlXk' 
                                                                         '\n'
...
def job_tue9(chat_id):
    bot.send_message(chat_id,  '🙋🏽‍♂Доброй ночи, надеюсь тебе понравилось! Завтра ждет третий день марафона и новая программа!')
# -------------------------------------------------------------------------------------------------------------------------
#TUESDAY SCHEDULE
