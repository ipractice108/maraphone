...
# SUNDAY SCHEDULE
# wake up
def job_sun1(chat_id):
    bot.send_message(chat_id, '🌝Добрейшего утра! \n'
                                                  '\n'
                                                  '😍Время просыпаться, запускать сознание и тело для новых свершений!\n'
                                                  '\n'
                                                  '😇Сегодня товой день "ВОЛЬНОГО ГОЛОДАНИЯ". Вытянись пальцами рук и пальцами ног, вытягивая в разные стороны позвоночник и мышцы тела.  Определи ногу с которой ты встаешь и вперед на водные процедуры.')

# почистить зубы
def job_sun2(chat_id):
    bot.send_message(chat_id, '🖖Пожалуйста возьми зубную щетку в непривычную, а может уже и ставшую более послушной и более привыноый для тебя руку. \n, '
...
# waterfasting
def job_sun3(chat_id):
    bot.send_message(chat_id, '🥛➕🥛➕🍋➕👌➕👌Пьем стакан воды комнатной температуры натощак и за ним второй с чистым лимонным соком, пищевой содой и щепотокой куркумы. Все как мы уже делали. Для любителей чистки как я, еще добавляю 5-8 капель перекиси водорода.')

def job_sun33(chat_id):
    bot.send_message(chat_id, '🥛➕🥛Если захочется есть, пьем еще воду, постарайтесь провести сегодняшний день на воде, максимум на разбавленных свежевыжатых соках')

# warm up + food notice
def job_sun4(chat_id):
    bot.send_message(chat_id, 'https://youtu.be/vIpOIkQq704')
    bot.send_message(chat_id, '🕙В 10.00 ты получишь нопоминание о режиме "голодания" + полезное видео, для отвлечения ума от еды.')

# warm up + food notice
def job_sun5(chat_id):
    bot.send_message(chat_id, '💓О полезных свойствах голодания.\n'
                                                ...
                                                '\n')
def job_sun55(chat_id): bot.send_message(chat_id, '🤦‍♂️Добавим к этим незаметным факторам, ежедневно влияющим на жизнь человека, еще и режим и состав питания. Из-за предпочтения выбирать пищу по вкусовым качествам, а не по составу и необходимости, для конкретной конституции тела и жизнедеятельности человека, желудки миллиардов людей страдают от ежедневного несварения и постоянной тяжелой работы, переваривания и проталкивания недостаточно прожеванной еды, сочетание которой невозможно расщепить и усвоить.\n'
                                                '\n'
...
def job_sun555(chat_id): bot.send_message(chat_id, 'https://youtu.be/9TRDubzDEBU')
def job_sun5555(chat_id): bot.send_message(chat_id, 'https: // youtu.be / iQzpExpsezI')

# lounch recipie
def job_sun6(chat_id):
    bot.send_message(chat_id, '☕️Сегодня вечером, перед сном выпей немного мятного или травяного чаю БЕЗ СЛАДОСТЕЙ>. \n'
                                                '\n'
...
# evening lesson
def job_sun7(chat_id):
    bot.send_message(chat_id, '🌞Напоминаю, сегодня вечером мятный или травяной чай БЕЗ СЛАДОСТЕЙ.')

def job_sun8(chat_id):
    bot.send_message(chat_id, 'https://youtu.be/vIpOIkQq704 \n'
                                                                                 '\n'
...
# have a good night
def job_sun9(chat_id):
    bot.send_message(chat_id, '🙋🏽‍♂Доброй ночи, надеюсь тебе понравилось! Теперь зможно приступить к регулярным online занятиям "ipractica.club!" и групповым занятям в классе')
#------------------------------------------------------------------------------------------------------------------------------
