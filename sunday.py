...
def job_sun3(chat_id):
    bot.send_message(chat_id, '🥛➕🥛➕🍋➕👌➕👌Выпейте стакан воды комнатной температуры натощак и за ним второй с чистым лимонным соком, пищевой содой и щепотокой куркумы. Все как мы уже делали. Для любителей чистки как я, еще добавляю 5-8 капель перекиси водорода.')

def job_sun33(chat_id):
    bot.send_message(chat_id, '🥛➕🥛Если захочется есть, попейте еще воды, постарайтесь провести сегодняшний день на воде, максимум на разбавленных свежевыжптых соках')

# warm up + food notice
def job_sun4(chat_id):
    bot.send_message(chat_id, 'https://youtu.be/vIpOIkQq704')
    bot.send_message(chat_id, '🕙В 10.00 вы получите нопоминание о режиме "голодания" + полезные активности, для отвлечения ума от еды.')
...
def job_sun7(chat_id):
    bot.send_message(chat_id, '🌞Напоминаю, сегодня вечером травяной настой или чай без сладостей.')
...
def job_sun9(chat_id):
    bot.send_message(chat_id, '🙋🏽‍♂Доброй ночи, надеюсь Вам понравилось! Теперь вы готовы к регулярным online занятиям "ipractica.club!" и групповым занятям в классе')
#------------------------------------------------------------------------------------------------------------------------------
